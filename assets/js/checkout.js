  ...
  div.innerHTML = `
        ...
        <div class="flex-1">
            ...
            <div class="flex items-center space-x-2 mt-1">
                <span class="font-medium text-amber-600">${formatCurrency(
                  discountedPrice
                )}</span>
                ${
                  hasDiscount
                    ? `<span class="text-xs text-gray-500 line-through">${formatCurrency(
                        price
                      )}</span>`
                    : ""
                }
...
function initEventListeners() {
  ...
  document
    .getElementById("same-as-billing")
  ...
  document
    ...
    .addEventListener("submit", handleCheckoutSubmission);
...
async function submitOrder(orderData) {
  try {
    ...
    const result = await window.ApiService.post("/orders", orderData);
