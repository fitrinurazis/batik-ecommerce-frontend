    ...
    tableBody.innerHTML = orders.map(order => {
        ...
        <tr class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                ...
                ${order.customer_phone ? `<div class="text-sm text-gray-500">${order.customer_phone}</div>` : ''}
            </td>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                ${paymentBadge}
                ${order.payment && (order.payment.payment_status === 'pending' || order.payment.status === 'pending') ? `
        ...
        contentDiv.innerHTML = `
            ...
            <div class="grid grid-cols-2 gap-4 text-sm mb-4">
                ...
                <div>
                    ...
                    <p class="font-medium text-green-600">${Utils.formatCurrency(payment.amount)}</p>
                </div>
                ${payment.bank_name ? `
                <div>
                ...
                <div>
                    ...
                    <p class="font-medium">${Utils.formatDateShort(payment.payment_date)}</p>
                </div>
                ` : ''}
                ${payment.created_at ? `
                <div>
                    <p class="text-gray-600 mb-1">Diupload:</p>
                    <p class="font-medium">${Utils.formatDateShort(payment.created_at)}</p>
                </div>
                ` : ''}
            </div>
            ...
            <div class="bg-green-50 border border-green-200 rounded-lg p-3 mb-3">
                ...
                <p class="text-xs text-green-700">Diverifikasi pada ${Utils.formatDate(payment.verified_at)}</p>
            </div>
            ` : ''}
            ...
            <div class="bg-red-50 border border-red-200 rounded-lg p-3 mb-3">
                ...
                <p class="text-sm text-red-800 mb-1"><strong>Alasan:</strong> ${payment.rejection_reason}</p>
                ${payment.verified_at ? `<p class="text-xs text-red-700">Ditolak pada ${Utils.formatDate(payment.verified_at)}</p>` : ''}
...
function openStatusModal(orderId, currentStatus) {
    ...
    document.getElementById('status-current-badge').innerHTML = Utils.getStatusBadge(currentStatus);
...
function getPaymentBadge(payment) {
    ...
    const statusConfig = {
        'pending': { class: 'bg-yellow-100 text-yellow-800', text: 'Menunggu Verifikasi', icon: 'clock' },
        'verified': { class: 'bg-green-100 text-green-800', text: 'Terverifikasi', icon: 'check-circle' },
        'rejected': { class: 'bg-red-100 text-red-800', text: 'Ditolak', icon: 'times-circle' }
    };
...
async function viewPaymentProof(orderId) {
    try {
        ...
        if (payment) {
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-medium">${getPaymentBadge(payment)}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="font-medium">${Utils.formatCurrency(payment.amount)}</p>
                        </div>
                        <div>
                    ...
                    ${payment.payment_proof ? `
                        <div>
                            ...
                            <img src="${getImageUrl(payment.payment_proof)}" alt="Bukti Transfer" class="max-w-full border rounded-lg shadow-sm" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22400%22 height=%22300%22%3E%3Crect fill=%22%23ddd%22 width=%22400%22 height=%22300%22/%3E%3Ctext fill=%22%23999%22 x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22%3EGambar tidak ditemukan%3C/text%3E%3C/svg%3E'">
                        </div>
                    ` : ''}
            ...
            contentDiv.innerHTML = '<p class="text-gray-500 text-center py-4">Belum ada bukti pembayaran</p>';
...
async function handleVerifyPayment() {
    ...
    try {
        ...
        Utils.showAlert('Pembayaran berhasil diverifikasi! Notifikasi telah dikirim ke pelanggan.', 'success');
        closeVerifyConfirmation();
        closePaymentModal();
...
async function handleRejectPayment() {
    ...
    try {
        ...
        Utils.showAlert('Pembayaran ditolak. Notifikasi telah dikirim ke pelanggan.', 'success');
        closeRejectModal();
        closePaymentModal();
