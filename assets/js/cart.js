...
function createCartItemElement(cartItem, product, index) {
    ...
    const fallbackImage = 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=200&h=200&fit=crop&crop=center&q=80';
    ...
    div.innerHTML = `
        <div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6">
            ...
            <div class="flex-shrink-0">
                <div class="w-24 h-24 md:w-32 md:h-32 bg-gray-200 rounded-lg overflow-hidden cursor-pointer" onclick="viewProduct(${product.id})">
            ...
            <div class="flex-1">
                <div class="flex flex-col md:flex-row md:justify-between">
                    ...
                    <div class="flex-1 mb-4 md:mb-0">
                        ...
                        <p class="text-gray-600 text-sm mb-2">${product.category || 'Batik'}</p>
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <div class="flex items-center border border-gray-300 rounded-lg">
                                <button onclick="updateQuantity(${index}, ${quantity - 1})"
                                        class="px-3 py-2 hover:bg-gray-100 transition-colors ${quantity <= 1 ? 'opacity-50 cursor-not-allowed' : ''}"
                                        ${quantity <= 1 ? 'disabled' : ''}>
                                    <i class="fas fa-minus text-sm"></i>
                                </button>
                                <input type="number" value="${quantity}" min="1" max="${product.stock || 999}"
                                       class="quantity-input w-16 text-center py-2 border-none focus:ring-0 focus:outline-none"
                                       onchange="updateQuantity(${index}, this.value)">
                                <button onclick="updateQuantity(${index}, ${quantity + 1})"
                                        class="px-3 py-2 hover:bg-gray-100 transition-colors ${quantity >= (product.stock || 999) ? 'opacity-50 cursor-not-allowed' : ''}"
                                        ${quantity >= (product.stock || 999) ? 'disabled' : ''}>
...
function confirmRemoveItem() {
    if (itemToRemove !== null) {
        ...
        const itemElement = document.querySelector(`[data-index="${itemToRemove}"]`);
        if (itemElement) {
...
function updateOrderSummary() {
    ...
    console.log('Order summary updated:', { subtotal, totalItems, totalDiscount, grandTotal });
}
    ...
    card.innerHTML = `
        ...
        <div class="p-4">
            ...
            <div class="flex items-center justify-between">
                ...
                <button onclick="addToCartFromRecommendation(${product.id})"
...
function initEventListeners() {
    ...
    document.getElementById('cancel-remove').addEventListener('click', hideRemoveModal);
